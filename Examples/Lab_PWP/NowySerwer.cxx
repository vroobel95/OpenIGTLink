...
int SendMessage(igtl::Socket * socket, igtl::PointElement::Pointer pointElement)
{
	igtl::PointMessage::Pointer pointMessage;

	pointMessage= igtl::PointMessage::New();
	pointMessage->SetDeviceName("PointSender");
	...
	pointMessage->AddPointElement(pointElement);
	pointMessage->Pack();

	igtl::Sleep(1000);

	int r = socket->Send(pointMessage->GetPackPointer(), pointMessage->GetPackSize());
...
int ReceivePoint(igtl::Socket * socket, igtl::MessageHeader * header)
{
  ...
  if (c & igtl::MessageHeader::UNPACK_BODY) // if CRC check is OK
    ...
    for (int i = 0; i < nElements; i ++)
      ...
      std::cerr << "================================" << std::endl;
	  SendMessage(socket, pointElement);
      }
    }
